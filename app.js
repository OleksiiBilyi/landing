...
let offset3D = 0;
const sliderLine3D = document.querySelector('.clothing-simulation__images-carousel');
document.querySelector('.next-btn-3d').addEventListener('click', function() {
  if(mediaQuery.matches) {
    document.querySelector('.next-btn-3d').classList.toggle('next-btn-3d--active');
    offset3D += 323;
    if (offset3D > 323) {
      offset3D = 0;
    }
    sliderLine3D.style.left = -offset3D + 'px';
  }  
    else {
        document.querySelector('.next-btn-3d').classList.toggle('next-btn-3d--active');
        offset3D += 623;
